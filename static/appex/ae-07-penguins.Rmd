...
## Questions about Lab 03 + team work? 
...
In this AE, we will look at the `penguins` data set from the [palmerpenguins R package](v) maintained by Dr. Allison Horst. This data contains measurements and other characteristics for 344 penguins observed near Palmer Station Antarctica. The data were originally collected by Dr. Kristen Gorman. 


We will use Analysis of Variance (ANOVA) to compare the mean body mass (`body_mass_g`), for the three penguin species (`species`) - Adélie, Chinstrap and Gentoo. 
...
We will use the ANOVA table to test the hypotheses. 
...
2. What is the sum of squares used to measure
...
3. State the conclusion in the context of the data. 

### Checking conditions 

4. Are the conditions for ANOVA satisfied? Use the plots and summary statistics from the EDA. You can create more plots and calculate additional summary statistics as needed.

- **Normality**
- **Constant variance**
- **Independence**

## Next steps: Pairwise comparisions  

Using the ANOVA test, we concluded that at least one species has a mean body mass that's different from the others. The next natural question is how does body mass compare across species. 

- Issue with multiple comparison 
  - Indvidiual vs. Family-wise error rate
- Bonferonni correction 
- Calculate multiple CI using Bonferonni correction 

```{r}
#install.packages(pairwiseCI, dependencies = TRUE)
# run in console to install  package

library(pairwiseCI) #show output and show how to interpret
pairwiseCI(body_mass_g ~ species, data = penguins, conf.level = 0.95)
```
