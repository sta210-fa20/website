...
## [Click here for PDF of slides](11-mlr-transformations.pdf)
...
- Given the previous facts, if $\hat{\log(Y)} = \hat{\beta}_0 + \hat{\beta}_1 x$, then 
.alert[
...
.vocab[Intercept]: The median respiratory rate for a new born child is `r round(exp(3.845),3)` (exp{3.845}) breaths per minute.
...
### Model with Transformation on $Z$
