...
$$\color{#87037B}{\log(\text{total}) = 1.302 - 0.0047 \times \text{age}}$$
...
For each additional year older the head of the household is, we expect the mean number in the house to multiply by a factor of `r round(exp(-0.0047),3)` (exp(-0.0.0047)). 

--

For households with a head of the household who is `r round(mean(hh$age),3)` years old, we expect the mean number of people in the household to be `r round(exp(1.302),3)` (exp(1.302)).
...
```{r echo=T}
model1 <- glm(total ~ ageCent, data = hh, family = "poisson")
model2 <- glm(total ~ ageCent + I(ageCent^2), data = hh, family = "poisson")
```
