...
- Conduct a hypothesis testing for $\beta_j$
...
The p-value is calculated using a $t$ distribution with $n-p-1$ degrees of freedom.

In this example, the p-value is calculated using a $t$ distribution $85-6-1 = 78$ degrees of freedom.

--

<font class = "vocab"> Given </font>  $\beta_{living\_area}  = 0$ </font class = "vocab"> the probability of observing a slope at least as extreme as the one we've observed, 65.903, is 0.00009.</font>
...
.eq[
$$\hat{\sigma}_\epsilon^2  = \frac{RSS}{n-p-1} = \frac{\sum_{i=1}^ne_i^2}{n-p-1}$$
]
